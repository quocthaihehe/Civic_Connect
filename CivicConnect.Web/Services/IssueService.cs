    ...
    public class IssueService : IIssueService
    {
        ...
        public async Task<bool> VoteIssueAsync(int issueId, string userId, VoteType type)
        {
            ...
            if (existingVote != null)
            {
                ...
                {
                    // Hủy vote nếu click lại cùng loại
            ...
            int issueId, string authorId, string content, int? parentCommentId, bool isOfficial)
        {
            ...
            // Cập nhật thời gian thay đổi phản ánh
            ...
            if (issue != null)
            {
                ...
                // Gửi thông báo cho người viết phản ánh nếu có comment mới từ cán bộ
                if (isOfficial && issue.AuthorId != authorId)
                {
                    await _notificationService.SendNotificationAsync(
                        issue.AuthorId,
                        "Cơ quan phản hồi phản ánh của bạn",
                        $"Cán bộ xử lý đã gửi một phản hồi chính thức cho phản ánh: '{issue.Title}'",
        ...
        public async Task<bool> RejectIssueAsync(int issueId, string officialId, string reason)
        {
            ...
            await _context.SaveChangesAsync();
            await SaveHistoryAsync(issueId, oldStatus, IssueStatus.Rejected, officialId, $"Từ chối phản ánh. Lý do: {reason}");
        ...
        public async Task<bool> EscalateIssueAsync(int issueId, string officialId, string note)
        {
            ...
            // Chỉ cho phép chuyển cấp từ Phường lên Quận
            ...
            var escalatedIssue = new Issue
            {
                ...
                IsVerified = true,
                ParentIssueId = issue.Id, // Liên kết về issue gốc
                CreatedAt = DateTime.UtcNow,
            ...
            // Sao chép hình ảnh sang issue mới
            ...
            // Ghi nhận lịch sử issue mới
            await SaveHistoryAsync(escalatedIssue.Id, IssueStatus.Pending, IssueStatus.Pending, officialId, $"Khởi tạo từ luồng chuyển cấp phản ánh #{issue.Id}.");
        ...
        public async Task<bool> AssignIssueAsync(int issueId, string officialId, string assignedToUserId, string? note)
        {
            ...
            await _context.SaveChangesAsync();
            await SaveHistoryAsync(issueId, oldStatus, IssueStatus.Assigned, officialId, note ?? $"Phân công cho cán bộ khác xử lý.");

            await _notificationService.SendNotificationAsync(
                assignedToUserId,
                "Bạn được phân công một phản ánh",
                $"Bạn được phân công phụ trách giải quyết phản ánh: '{issue.Title}'",
        ...
        {
            // Tính số lượng issue khác trong cùng bán kính 2km làm trọng số density
        ...
        private double CalculateDistance(double lat1, double lon1, double lat2, double lon2)
        {
            ...
            return 6371.0 * c; // Khoảng cách theo km
        }
        ...
        private DateTime CalculateDueDate(IssuePriority priority)
        {
            ...
            {
                IssuePriority.Critical => 3,   // 3 ngày làm việc
                IssuePriority.High => 7,       // 7 ngày làm việc
                IssuePriority.Medium => 15,    // 15 ngày làm việc
                IssuePriority.Low => 30,       // 30 ngày làm việc
                _ => 15
            };
